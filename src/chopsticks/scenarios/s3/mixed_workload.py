...
uploaded_objects = {"small": [], "medium": [], "large": []}
...
def on_test_start(environment, **kwargs):
    ...
    print(f"\n{'='*80}")
    print(f"Starting S3 Mixed Workload Stress Test")
    print(f"{'='*80}")
    print(f"Object Size Distribution:")
    for size_class, config in TEST_CONFIG["object_size_distribution"].items():
        print(f"  {size_class}: {config['weight']}% ({config['min_kb']}KB - {config['max_kb']}KB)")
...
class MixedWorkloadUser(S3Workload):
    ...
    def delete_object_mixed(self):
        ...
        available_size_classes = [
            sc for sc in ["small", "medium", "large"]

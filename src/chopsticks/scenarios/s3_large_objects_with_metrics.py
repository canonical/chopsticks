...
def on_locust_init(environment, **kwargs):
    ...
    test_config = TestConfiguration(
        ...
        test_config={
            ...
            "users": environment.parsed_options.num_users if hasattr(environment, 'parsed_options') else 1,
        }
    )
    ...
    print(f"\n{'='*70}")
    print(f"Metrics Collection Initialized")
    print(f"{'='*70}")
    print(f"Test Run ID: {test_config.test_run_id}")
    print(f"Chopsticks Metrics Endpoint: http://0.0.0.0:8090/metrics")
    print(f"{'='*70}\n")
...
def on_locust_quit(environment, **kwargs):
    ...
    if metrics_collector:
        ...
        test_config.duration_seconds = int((test_config.end_time - test_config.start_time).total_seconds())
        
        # Export metrics
        output_dir = "/tmp/chopsticks_metrics"
        os.makedirs(output_dir, exist_ok=True)
        
        metrics_collector.export_json(f"{output_dir}/metrics_{test_config.test_run_id}.json")
        metrics_collector.export_csv(f"{output_dir}/metrics_{test_config.test_run_id}.csv")
        metrics_collector.export_jsonl(f"{output_dir}/metrics_{test_config.test_run_id}.jsonl")
...
class S3LargeObjectsWithMetrics(S3Workload):
    ...
    def _record_metric(self, operation_type: OperationType, key: str, 
                       size_bytes: int, start_time: datetime, 
    ...
    def upload_large_object(self):
        ...
        start_time = datetime.utcnow()
        try:
            result = self.client.upload(key, data)
            end_time = datetime.utcnow()
            self._record_metric(OperationType.UPLOAD, key, self.object_size, 
    ...
    def download_large_object(self):
        ...
        try:
            ...
            end_time = datetime.utcnow()
            self._record_metric(OperationType.DOWNLOAD, key, len(data),
                              start_time, end_time, True)
            
            # Verify data size
            if len(data) != self.object_size:
                print(f"Warning: Downloaded size {len(data)} != expected {self.object_size}")
        except Exception as e:
            end_time = datetime.utcnow()
            self._record_metric(OperationType.DOWNLOAD, key, self.object_size,
                              start_time, end_time, False,
                              error_code=type(e).__name__, error_msg=str(e))
            raise
    ...
    def delete_object(self):
        ...
        try:
            ...
            end_time = datetime.utcnow()
            self._record_metric(OperationType.DELETE, key, 0,
                              start_time, end_time, True)
        except Exception as e:
            end_time = datetime.utcnow()
            self._record_metric(OperationType.DELETE, key, 0,
